...
def make_padding_masks(x, wav_len):
    # Batch-one full-length input only; no padded batching.
    assert x.ndim == 2 and x.shape[0] == 1 and float(wav_len[0]) == 1.0
...
class WavLMWrapper(
    ...
    def __init__(
        ...
        if self.pretrain_model == "wavlm":
            ...
            self.processor = Wav2Vec2FeatureExtractor.from_pretrained('microsoft/wavlm-large', revision='c1423ed94bb01d80a3f5ce5bc39f6026a0f4828c')
            self.backbone_model = WavLMModel(WavLMConfig.from_pretrained("microsoft/wavlm-large", revision="c1423ed94bb01d80a3f5ce5bc39f6026a0f4828c", output_hidden_states=True))
        ...
        if self.pretrain_model == "wavlm_large":  
            with torch.no_grad():
                ...
                for idx in range(len(x)):
                    input = self.processor(x[idx].detach().cpu().numpy(), sampling_rate=16_000, return_tensors="pt", padding=False)
