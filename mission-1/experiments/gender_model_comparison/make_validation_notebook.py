"""Generate the complete Colab notebook and a matching percent-format Python file."""
import ast
import json
...
def make_notebook(bundle_sha):
    cells = []
    def md(s):
        cells.append(dict(cell_type='markdown', metadata={}, source=s.splitlines(keepends=True)))
    def code(s):
        ast.parse(s)
        cells.append(dict(cell_type='code', metadata={}, source=s.splitlines(keepends=True), execution_count=None, outputs=[]))
    md('''# Mission 1 · 고정 ECAPA Internal Validation 5,597통화
...
''')
    md('## 1. Drive 연결 · 별도 Validation 코드 설치\n기존 Train 코드·config·결과를 덮어쓰지 않습니다.')
...
EXPECTED_SHA256 = ''' + repr(bundle_sha) + '''
...
''')
    code('''def release_stale_lock(expected_token, previous_runtime_stopped=False):
...
''')
    ...
    nb = dict(nbformat=4, nbformat_minor=5, cells=cells,
              metadata=dict(kernelspec=dict(name='python3', display_name='Python 3', language='python'),
                            language_info=dict(name='python'), colab=dict(name='ECAPA_Internal_Validation.ipynb', provenance=[])))
    (ROOT/'ECAPA_Internal_Validation.ipynb').write_text(json.dumps(nb, ensure_ascii=False, indent=2)+'\n')
    text = []
    for c in cells:
        if c['cell_type'] == 'markdown':
            text.append('# %% [markdown]\n'+'\n'.join(('# '+l).rstrip() for l in ''.join(c['source']).splitlines()))
        else:
            text.append('# %%\n'+''.join(c['source']))
    (ROOT/'ECAPA_VALIDATION_COLAB.py').write_text('\n\n'.join(text).rstrip()+'\n')
